...
def point_size_for_pixel_size(pixel_size: float) -> float:
    return round(pixel_size * 72 / 96, 2)


def _font_size_css(pixel_size: float) -> str:
    point_size = point_size_for_pixel_size(pixel_size)
    text = f"{point_size:.2f}".rstrip("0").rstrip(".")
    return f"{text}pt"
...
def _stylesheet(palette: ThemePalette) -> str:
    ...
    if palette.name == "day":
        ...
        table_body_fill = "#f5f2ee"
        table_header_fill = "#efede9"
    else:
        ...
        table_body_fill = c.surface_alt
        table_header_fill = c.surface_elevated
    selected_fill = c.selection
    return f"""
QWidget {{
    font-size: {_font_size_css(t.body_size)};
    color: {c.text};
}}
...
#ResourceHeaderTitle {{
    color: {c.text};
    font-size: {_font_size_css(t.title_size - 2)};
    font-weight: 700;
}}
#ResourceHeaderSubtitle, #ResourceSpecMeta {{
    color: {c.text_muted};
    font-size: {_font_size_css(t.meta_size + 1)};
}}
#ResourcePaneTitle, #ResourceSpecTitle {{
    color: {c.text};
    font-size: {_font_size_css(t.body_size + 2)};
    font-weight: 700;
}}
#ResourceSearchInput {{
    background: {c.input};
...
#ResourceFontResolved {{
    font-size: {_font_size_css(t.meta_size)};
}}
...
#ResourceColorLabTitle {{
    color: {c.text};
    font-size: {_font_size_css(t.body_size)};
    font-weight: 700;
}}
#ResourceColorLabSwatch {{
    border: 1px solid {c.border_strong};
    border-radius: 9px;
    font-size: {_font_size_css(t.meta_size + 1)};
    font-weight: 700;
...
#ResourceColorLabInput:focus {{
    border-color: {c.border_strong};
    background: {c.surface};
}}
#ResourceColorLabInput[invalid="true"] {{
    border-color: {state.error};
    color: {state.error};
}}
#ResourcePickColorButton, #ResourceCopyHexButton, #ResourceCopyRgbButton {{
    padding: 7px 9px;
    border-radius: {r.chip}px;
    font-size: {_font_size_css(t.meta_size + 1)};
}}
...
#Brand {{
    color: {c.text};
    font-size: {_font_size_css(t.title_size)};
    font-weight: 700;
...
#SectionLabel {{
    color: {c.text_soft};
    font-size: {_font_size_css(t.section_size)};
    font-weight: 700;
...
#InspectorStatus {{
    font-size: {_font_size_css(t.body_size + 1)};
}}
...
#ChatParagraphText, #ChatListItemText, #ChatQuoteBlock QLabel, #ChatListItem QLabel {{
    color: {c.text};
    font-size: {_font_size_css(t.body_size)};
    line-height: 1.48;
}}
...
#ChatHeadingBlock[level="1"] {{
    font-size: {_font_size_css(t.body_size + 5)};
    font-weight: 700;
}}
#ChatHeadingBlock[level="2"] {{
    font-size: {_font_size_css(t.body_size + 3)};
    font-weight: 700;
}}
...
#ToolStatusToggle {{
    ...
    border: 0;
    color: {c.text_muted};
    font-size: {_font_size_css(t.meta_size + 2)};
    font-weight: 600;
    padding: 0;
    min-width: 0;
}}
