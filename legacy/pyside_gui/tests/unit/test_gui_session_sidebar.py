...
class GuiSessionSidebarTests(unittest.TestCase):
    ...
    def test_collapsed_sidebar_shows_only_expand_button(self) -> None:
        sidebar = SessionSidebar()

        sidebar.set_collapsed(True)

        self.assertFalse(sidebar.collapse_button.isHidden())
        self.assertEqual(sidebar.collapse_button.toolTip(), "Expand history")
        self.assertTrue(sidebar.brand_title.isHidden())
        self.assertTrue(sidebar.workspace.isHidden())
        self.assertTrue(sidebar.new_button.isHidden())
        self.assertTrue(sidebar.section_label.isHidden())
        self.assertTrue(sidebar.session_tree.isHidden())
        self.assertTrue(sidebar.info_card.isHidden())
        self.assertTrue(sidebar.project_button.isHidden())
        self.assertTrue(sidebar.day_button.isHidden())
        self.assertTrue(sidebar.night_button.isHidden())
        self.assertTrue(sidebar.settings_button.isHidden())
    ...
    def test_session_row_title_elides_before_touching_label_edge(self) -> None:
        sidebar = SessionSidebar()
        sidebar.resize(360, 720)
        sidebar.show()
        self.app.processEvents()
        sidebar.set_session_groups(
            [
                SessionGroupRecord(
                    scope=_scope("conversation", "瀵硅瘽"),
                    records=[
                        _record(
                            "chat-one",
                            "这是一个很长很长的用户问题标题用来验证右侧留白不会贴着边框显示",
                        )
                    ],
                )
            ],
        ...
        row = sidebar._group_sections[0].session_row(0)
        assert row is not None
        title = row.findChild(QLabel, "SessionRowTitle")
        assert title is not None
        metrics = title.fontMetrics()

        self.assertTrue(title.text().endswith("…"))
        self.assertLessEqual(metrics.horizontalAdvance(title.text()), title.width() - 20)
