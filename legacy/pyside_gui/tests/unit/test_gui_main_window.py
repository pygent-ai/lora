...
class GuiMainWindowTests(unittest.TestCase):
    ...
    def test_send_message_persists_session_title_from_first_prompt(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            window = MainWindow(workspace_root=tmp)
            session_id = window.store.create_chat_session().session_id
            window.select_session(session_id)
            window._start_chat_worker = lambda *_args: None

            window.send_message("第一行问题\n第二行问题")

            records = window.store.list_chat_sessions()
