...
def _case_run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    session_id = getattr(args, "session_id", None) or case.session.get("session_id") or case.session.get("id")
    if session_id is None:
        session_id = manager.create(case.id, mode=case.type).session_id
