...
class SessionManager:
    ...
    def create(self, case_id: str, mode: str = "e2e") -> SessionRef:
        ...
        self._write_json(self._pygent_session_path(session_id), session.to_dict())
    ...
    def fork(self, source_session_id: str) -> SessionRef:
        ...
        source_meta = self._read_json(source_dir / "metadata.json")
        ...
        data = self._read_json(target_dir / "session.json")
        data.update(
            {
    ...
    def start_case_run(self, session_id: str, case_id: str, run_config: RunConfig | None = None) -> CaseRunRef:
        ...
        self._write_json(run_dir / "run_config.json", config.to_dict())
        self._write_json(
            run_dir / "run_metadata.json",
            {"status": "running", "started_at": _now(), **ref.to_dict()},
        )
    ...
    def finish_case_run(self, case_run_ref: CaseRunRef, status: str) -> None:
        ...
        metadata.update({"status": status, "finished_at": _now()})
    ...
    def show(self, session_id: str) -> dict[str, Any]:
        ...
        metadata = self._read_json(self._session_dir(session_id) / "metadata.json")
    ...
    def _new_session_id(self, case_id: str, mode: str) -> str:
        ...
        digest = hashlib.sha1(f"{case_id}:{mode}:{stamp}:{_now()}".encode("utf-8")).hexdigest()[:6]
    ...
    def _append_session_event(self, session_id: str, event_type: str, payload: dict[str, Any]) -> None:
        ...
        event = {"type": event_type, "timestamp": _now(), "session_id": session_id, "payload": payload}
