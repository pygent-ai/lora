...
Lora 是基于 Pygent 0.2.3 的本地 Agent 开发与评测工具。API、CLI、case runner 共用 workspace 级 `LoraRuntimeService`，执行、并发、持久化、模型路由、工具任务和审批均由 Pygent Runtime 管理。
...
runtime:
  durability:
    mode: preferred
    history_path: .lora/runtime/executions.sqlite3
  capacity:
    scope: runtime_instance
    coordinator_path: .lora/runtime/capacity.sqlite3
  approvals:
    enabled: true
    timeout_seconds: 300
    preauthorized_tools: []
